...
// The referral this build credits when the local fleet has no eligible referrer of its own.
//
// Kindra pays a referrer 500 gold + 200 on-chain $KINDRA for every character that joins through
// them and reaches account Lv 5, and gives the newcomer 250 gold. A fresh install has no character
// at the Lv-10 referring threshold yet, so without a default that reward is simply never paid to
// anyone. This build credits the project's own character in that case.
//
// It is deliberately a plain, visible constant rather than something buried: override it with
// KINDRA_REFERRER=<character name> in .env, or set it to '' to disable referral attribution
// entirely. Once YOUR fleet has a character at account Lv 10, your own characters take over
// automatically — see Fleet.bestReferrer().
export const DEFAULT_REFERRER = process.env.KINDRA_REFERRER !== undefined
  ? process.env.KINDRA_REFERRER
  : 'Dainfield';
