...
export const CFG = {
  ...
  // ~11k/day ceiling is most of a day of farming sitting in-game as soft currency.
  //
  // 300 is deliberately near the floor: the point is that gold does not accumulate, it becomes
  // token. A character lists as soon as it holds 1,300.
  //
  // Spending still comes first, which is what makes a reserve this small survivable: inside one
  // sell trip the order is rations -> repairs -> upgrades -> cash-out, so the listing only ever
  // takes what is left after the character has bought what it needed. What 300 does not leave is
  // a *savings* balance — anything costing more than about a lot's worth will never be reached,
  // because the surplus is listed before it can pile up. Raise KINDRA_GOLD_RESERVE when that
  // starts to matter.
  goldReserve: Math.max(0, parseInt(process.env.KINDRA_GOLD_RESERVE || '300', 10) || 0),
