...
const HP_FLOOR = 0.65;    // same travel-safe margin the orchestrator uses; gathering walks just as far
const NODE_RANGE = 2.2;   // server gates at 3.0; we stand well inside so leash skew can't cross it

export class Gatherer {
  constructor({ net, state, move, economy = null, log = console.log }) {
    this.net = net; this.state = state; this.move = move; this.economy = economy; this.log = log;
    // Set with the fleet's mode. It decides what a node is worth to us: experience, or money.
    this.goldMode = false;
    this.onHurt = null;         // set by the orchestrator: step out of aggro and regen mid-gather
  ...
  pickNode(skill, candidates) {
    ...
    for (const n of candidates) {
      ...
      // What this node is WORTH, not just what it teaches. Scoring on xp alone sent the character
      // to whichever node taught fastest regardless of what it yielded — a motherlode drops ore
      // worth 2 gold and takes 34 swings, an amber_deposit drops amber worth 9 and takes 7, and on
      // xp-per-second the motherlode won. In gold mode experience is priced at almost nothing, so
      // the walk goes to the money.
      const rate = this.goldMode ? 0.02 : 0.25;
      const drop = def.drop || def.item;
      const dropValue = drop && this.economy
        ? hits * Math.max(this.economy.vendorValue(drop) || 0, this.economy.marketValue(drop) || 0)
        : 0;
      const worth = ((def.gold || 0) + dropValue + (def.baseXp || 0) * hits * rate) * mult;
      const travel = Math.hypot(n.x - me.x, n.z - me.z) / this.move.speed;
      const score = worth / (travel + hits * interval + 0.5);
  ...
  async run(skill, { stopWhen = () => false, maxMs = 15 * 60 * 1000, only = null } = {}) {
    ...
    while (!this._stop && Date.now() - started < maxMs) {
      ...
      // A gather cycle can run for minutes, walking 30-170u between nodes through a hostile region.
      // Nothing here reads hp, so the character is chewed on the whole way and only finds out when
      // the orchestrator hands it to combat — which is how kindra-20 died a minute into mining.
      // The orchestrator wires onHurt to combat.recover().
      if (this.onHurt && (this.state.me?.hp ?? 100) / 100 < HP_FLOOR) await this.onHurt();
