...
export class Economy {
  ...
  // What a market sale nets after the 8% tax and the 1% listing fee. Priced off goingRate() for
  // the same reason listValuables is: valuing a stack at the cheapest ask on a 125-deep book said
  // mythril was worth 28g when Vlad paid 38g and the going rate was 86g, so route() sent it to the
  // wrong counter before listValuables ever got a look at it.
  marketValue(item) {
    const px = this.goingRate(item);
  ...
  // What this item is really worth on the board, and how crowded that board is.
  //
  // The cheapest ask is the wrong anchor on a deep book: one desperate seller sets it, and matching
  // them means joining a queue of hundreds at a price nobody had to accept. The median is the wrong
  // anchor too — sitting at the median behind 500 queued units never clears. So take the cheap END
  // of the book without taking its floor: the 25th percentile, then a hair under it.
  bookFor(item) {
    const px = [];
    let units = 0;
    ...
    px.sort((a, b) => a - b);
    return { px, units, depth: px.length };
  }

  // The price to actually list at. Undercuts the cheap quarter of the book by a hair, so we are
  // near the front of the queue without racing anyone to 1g. With no book at all, fall back to the
  // server's own reference price rather than the item's base value — base is what a shop CHARGES,
  // and listing at it is how a stack sits untouched for a day.
  goingRate(item) {
    const { px, depth } = this.bookFor(item);
    if (!depth) {
      const ref = this.state.marketPrices[item] || 0;
      return ref > 0 ? Math.max(MARKET.minPrice, Math.floor(ref * 0.9))
                     : Math.max(MARKET.minPrice, Math.floor((ITEMS[item]?.sell || 1) * 0.6));
    }
    const q = px[Math.floor((depth - 1) * 0.25)];
    // A hair under, and at least 1g under, so we are ahead of everything priced at the same number.
    return Math.max(MARKET.minPrice, Math.min(q - 1, Math.floor(q * 0.97)));
  }

  // Cheapest live ask for an item.
  bestAsk(item) {
    let best = Infinity;
    for (const l of this.state.market) {
      if (l.item !== item || l.cur !== 'gold' || l.kind !== 'item') continue;
  ...
  async listValuables({ maxListings = 5 } = {}) {
    ...
    for (const [item, qty] of Object.entries(inv)) {
      ...
      // Never undercut below the alternative that is ACTUALLY available. Two things were wrong here.
      //
      // First, the floor was `vendorValue()` — what Vlad pays when he is open. Once the daily cap is
      // spent he pays nothing at all, so the check compared a real market price against a price that
      // did not exist, refused the sale, and banked the stack for a counter that would not reopen
      // until the next UTC day. 366 refusals in one seven-hour run, every one of them against a
      // phantom bid.
      //
      // Second, `ask - 1` chased the BOTTOM of the book. mythril_nugget has 125 listings: cheapest
      // 20g, median 86g, and Vlad pays 38g. Undercutting the cheapest put us at 28g — worse than the
      // vendor AND worse than two thirds of the book, on an item with 544 units already queued ahead
      // of us. Being the cheapest of 125 is not a sale, it is a donation. Price against the going
      // rate instead and let `goingRate()` decide what that is.
      const floor = this.vendorRoom() > 0 ? this.vendorValue(item) : 0;
      const wanted = this.goingRate(item);
      if (wanted <= floor) {
        this.log(`[econ] keeping ${qty}× ${item}: the market pays ${wanted}g, the vendor ${floor}g`);
        continue;
      }
      ...
      put.push({ item, qty, price });
      listed++;
    }
    // One summary line per trip, not one per stack. Five separate "listed 12× x" lines per sell
    // trip across a fifty-character fleet is a notification every few seconds; the trip total is
    // the thing a person actually wants pushed to them.
    if (put.length) {
      const worth = put.reduce((a, p) => a + p.price * p.qty, 0);
      const what = put.map((p) => `${p.qty}× ${p.item} @ ${p.price}g`).join(', ');
      this.log(`[econ] market: listed ${put.length} stack(s) — ${what} (~${worth}g if they clear)`);
    }
    return listed;
  }
  ...
  async fillBuyOrders({ minEdge = 1.15 } = {}) {
    ...
    for (const [item, qty] of Object.entries(inv)) {
      ...
      // `fillOrder`, not `fillBuyOrder`. The ruleset's own gold-history label table names the three
      // buy-order verbs — buyOrder / fillOrder / cancelOrder — and we were sending a fourth that
      // does not exist. The server drops an unknown verb silently, and this logs its success
      // optimistically, so every fill we ever "made" was a line in the log and nothing else.
      this.net.send({ t: 'fillOrder', id: order.id, qty: take });
      await sleep(500);
      filled.push({ item, qty: take, price: order.price, alternative });
      // "sent", not "filled". Nothing here waits for an acknowledgement, and the old wording is how
      // a verb the server never understood read as a working feature for a day. A real fill shows
      // up as gold on the `sales` handler above; if these lines appear and the balance does not
      // move, the verb is wrong again.
      this.log(`[econ] buy order: sent ${take}× ${item} @ ${order.price}g (else ~${alternative}g)`);
    }
    return filled;
  }
