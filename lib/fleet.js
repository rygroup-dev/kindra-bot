...
export class Fleet {
  ...
  onlineIn(group) { return group.filter((b) => ['online', 'running'].includes(b.status)).length; }
  ...
  startShifts({ everyMs = 45 * 60 * 1000 } = {}) {
    ...
    const rotate = async () => {
      for (const [, group] of this.ipGroups()) {
        ...
        for (const b of group) if (['online', 'running'].includes(b.status)) b.disconnect();
  ...
  get online() { return [...this.bots.values()].filter((b) => ['online', 'running'].includes(b.status)).length; }
