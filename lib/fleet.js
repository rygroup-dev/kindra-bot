...
import { KINSHIP_RADIUS as KINSHIP_R, BOSSES, bossXp, realmAt, accountLevel, accountXpOf } from './rules.js';

const XP_GOLD = 0.25;   // the same exchange rate the orchestrator prices xp at

// Account Lv 10 opens two realm portals. It is what /pin chases, and the only reason the fleet
// still tracks account level at all.
const ACCOUNT_GATE_LV = 10;
...
export class Fleet {
  ...
  //
  // A new character is minted with NO referrer, and there is no way to give it one. The join frame
  // carries no `ref` at all. This is deliberate: crediting a referrer is worth 500 gold + 200
  // $KINDRA, and doing it from your own fleet cost this project twelve accounts — the eleven that
  // converted and the character that collected, all banned for a year in one night.
  static createWallets(n, { world = 'valley', startIndex = null, character = null } = {}) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      made.push({ label, privateKey: pk, address: acct.address, world, name: '', character: cls, createdAt: new Date().toISOString() });
    }
    Fleet.saveWallets([...existing, ...made]);
    return made;
  }

  // Remember each character's in-game name once it joins: the server generates it from the wallet,
  // and persisting it is what lets the panel and the logs call a character by name while it is
  // off-shift and has no live state to read.
  rememberNames() {
  ...
  add(entry) {
    ...
    // A ban outlives the process by about a year, so it has to be remembered in the wallet book.
    // Restoring it here is what stops a restart from cheerfully queueing twelve dead accounts again.
    if (entry.bannedUntil && entry.bannedUntil > Date.now()) {
      bot.banned = { at: entry.bannedAt || 0, until: entry.bannedUntil, hours: Math.round((entry.bannedUntil - Date.now()) / 3600_000), reason: entry.bannedReason || 'banned' };
      bot.status = 'banned';
      bot.lastError = bot.banned.reason;
    }
    bot.on('banned', (b) => this.recordBan(entry.label, b));
  ...
  load() {
    ...
    // The primary is the fleet's TREASURY — sweeps land in it and gas is paid out of it — and that
    // is all it is. It used to be put into account-level chase mode automatically, because it was
    // also meant to reach the Lv-10 referring gate. With referrals gone that only made one
    // character farm combat at a quarter of the fleet's gold rate for nothing. Chase is now opt-in
    // through /pin.
    const primary = book.find((w) => w.primary) || book[0];
    if (primary) {
      const b = this.bots.get(primary.label);
      if (b) b.isPrimary = true;
    }
    this.log(`loaded ${this.bots.size} account(s) from ${WALLET_FILE}`);
    return [...this.bots.values()];
  }

  // Write a ban into the wallet book so it survives a restart, and say so once.
  recordBan(label, b) {
    const book = Fleet.loadWallets();
    const e = book.find((w) => w.label === label);
    if (!e) return;
    e.banned = true;
    e.bannedAt = b.at;
    e.bannedUntil = b.until;
    e.bannedReason = b.reason;
    Fleet.saveWallets(book);
    this.log(`${label} is banned until ${new Date(b.until).toISOString().slice(0, 10)} — removed from the rotation`);
  }

  // Banned accounts are excluded from EVERYTHING that hands out a slot. A ban runs for the better
  // part of a year, so this is not a backoff — it is a permanent removal that expires on its own.
  playable(b) {
    if (!b.banned) return true;
    if (b.banned.until && b.banned.until <= Date.now()) { b.banned = null; return true; }
    return false;
  }

  banned() { return [...this.bots.values()].filter((b) => !this.playable(b)); }
  ...
  //
  // The banned are filtered out here rather than at each call site: this one function feeds
  // _startAll, the shift rotation and the per-IP online count, and a dead account left in the list
  // would otherwise be handed a slot by whichever path was missed.
  ipGroups() {
    const groups = new Map();
    for (const b of this.bots.values()) {
      if (!this.playable(b)) continue;
  ...
  // account Lv 10, which is what opens the last two realm portals. Account xp weights combat at 1.0 and every
  ...
  pin(label) {
    ...
    this.saveRunState();
    this.log(`pinned ${label} — it holds a slot until account Lv ${ACCOUNT_GATE_LV}; the other ${MAX_ONLINE_PER_IP - 1} rotate`);
    return { ok: true };
  }
  ...
  releasePinIfDone() {
    ...
    if (!b?.state?.me) return false;
    const tl = b.state.me.tl;
    if (Number.isFinite(tl) && tl >= ACCOUNT_GATE_LV) {
      this.unpin({ reason: `it reached account Lv ${tl}` });
      return true;
    }
    // DEADLOCK, and one I built: `tl` is the server's account level and it arrives ONLY on `init`
    // — nothing updates it in session, not even the real client. A pinned character never rejoins,
    // because not rejoining is the entire point of pinning it. So the release condition was reading
    // a number frozen at the moment of the pin, and could never come true. kindra-01 sat at a
    // computed 100.0% for hours with `tl` still saying 9.
    //
    // We track the xp ourselves now, so we can tell when we have probably arrived — but "probably"
    // is not good enough to unpin on, and our count runs slightly ahead between `inv` frames. So
    // don't guess: bounce the socket, let `init` bring the server's own number, and decide on that.
    const mine = accountLevel(accountXpOf(b.state.me.skills || {}));
    if (mine < ACCOUNT_GATE_LV) return false;
    if (Date.now() - (this._pinProbeAt || 0) < 5 * 60 * 1000) return false;
    this._pinProbeAt = Date.now();
    this.log(`pin: our own count reads account Lv ${mine} while the server still says ${tl ?? '?'} — rejoining to ask it`);
    b.net.bounce();
    return false;
  }
  ...
  startShifts({ everyMs = 45 * 60 * 1000, checkEveryMs = 10 * 60 * 1000 } = {}) {
    ...
    // Do NOT zero the cursor. resume() restores it from disk precisely so a deploy continues the
    // circuit, and then this line threw it away — every restart sent the rotation back to wallet 1
    // while the tail of the book waited another full lap. With a dozen restarts in a day the last
    // accounts are reached rarely, which is the exact starvation the round robin replaced.
    this._shiftCursor = this._shiftCursor || 0;
    const rotate = async () => {
  ...
  async resume() {
    ...
    if (st.pinned && this.bots.has(st.pinned)) {
      ...
      if (b?.orch) b.orch.chaseAccountLevel = true;
      this.log(`resuming the pin on ${st.pinned} — still chasing account Lv ${ACCOUNT_GATE_LV}`);
    }
  ...
  // Pick up accounts added to wallets.json since we started. The book was read ONCE at boot, so a
  // wallet minted any other way — a script, an editor, a second session — stayed invisible to the
  // running process: twenty new accounts sat in the file for four minutes with the fleet still
  // reporting 24, not queued, not rotating, not playing. Minting from Telegram enrols them itself;
  // this catches every other route.
  syncBook() {
    let added = 0;
    for (const e of Fleet.loadWallets()) {
      if (this.bots.has(e.label)) continue;
      this.add(e);
      added++;
    }
    if (added) {
      this.log(`${added} new account(s) found in the wallet book — they join the rotation in turn`);
      this.saveRunState();
    }
    return added;
  }
  ...
  chooseRaid() {
    ...
    // Every boss, not just the paying ones. Filtering on `goldRoll` hid the Grove Warden — req
    // combat 0, 900 xp, 2.5-minute respawn — which is the only rung a Lv-1 character can stand on
    // and the first step of the ladder to a 5,000/day purse. Twenty accounts sat between Lv 1 and 8
    // unable to scratch a paying boss because the one they COULD fight was never offered.
    const golden = Object.entries(BOSSES)
      .sort((a, b) => (a[1].reqCombat ?? 0) - (b[1].reqCombat ?? 0));
    for (const [id, def] of golden) {
      const roll = def.reward?.goldRoll || null;
      ...
      // Somebody has to have purse room. This applies to the gateway bosses too, even though they
      // never pay a coin: the ONLY reason to fight one is to reach the purse, so with the day's
      // 5,000 already spent there is nothing at the top of the ladder to climb toward. Walking a
      // party across the map for raw xp is the "fattest health bar wins" behaviour this rewrite
      // exists to remove.
      ...
      const pay = roll ? (roll[0] + roll[1]) / 2 : 0;
      ...
      // The gate this kill opens, priced per crew member and averaged. A Warden run pays nobody a
      // coin, but if it is four kills from combat Lv 12 for everyone in the crew then each kill is
      // buying a quarter of the raise the Drowned King pays — which is the honest number to compare
      // against what they would otherwise be earning.
      const groupDps = safe.crew.reduce((a, b) => a + b.bosses.dps(), 0);
      // The climb is only worth making toward a purse that still has room today. With the day's
      // 5,000 already spent there is nothing at the top of the ladder to spend it on, so a gateway
      // boss that pays no gold scores nothing and the crew keeps earning where they are.
      const purse = Math.max(...safe.crew.map((b) => b.orch?.capLeft?.('boss') ?? 0));
      const unlock = safe.crew.reduce((a, b) => a + b.bosses.worth(def, { groupDps, capLeft: purse }).unlock, 0) / safe.crew.length;
      const perMin = (pay + share * XP_GOLD) * 60 / Math.max(1, safe.secs + travel) + unlock;
      ...
      // A fight worth nothing is not worth walking to, whatever the crew's alternative is. Without
      // this a gateway boss with the purse spent — pay 0, unlock 0 — scored 0 and still beat an
      // idle crew's alternative of 0, so the fleet trekked out for a gem and a mat.
      if (perMin <= 0) { blocked(`${id} pays nothing right now`); continue; }
      ...
      if (!best || perMin > best.perMin) {
        best = { bossId: id, x: def.x, z: def.z, members: safe.crew.map((b) => b.label),
                 perMin, pay, unlock, secs: safe.secs, travel, go: false, at: Date.now() };
      }
    }
    return best;
  }
