...
import { KINSHIP_RADIUS as KINSHIP_R, REFERRAL, BOSSES, bossXp, realmAt, accountLevel, accountXpOf } from './rules.js';
...
export class Fleet {
  ...
  //
  // `referrer` is left null unless the operator names one explicitly (KINDRA_REFERRER in .env).
  // It is sent as `join.ref` on that character's FIRST connect and attributed once, server-side.
  //
  // Do not point it at your own fleet. Referring yourself from one IP is not a loophole, it is the
  // most legible abuse pattern there is: eleven of this fleet's characters converted against its
  // own referrer and every one of them — plus the referrer — was banned for a year the same night.
  ...
  // The referrer to credit on a new character, or null. This USED to scan the fleet for whichever
  // of our own characters had cleared the Lv-10 gate and quietly credit that one — which is exactly
  // the self-referral the game bans for. It now returns only what the operator configured by hand,
  // and nothing at all by default.
  bestReferrer() {
    return DEFAULT_REFERRER ? { name: DEFAULT_REFERRER, tl: null, label: 'configured', isDefault: true } : null;
  }
  ...
  load() {
    ...
    // also the referrer-to-be; with referrals gone that only made one character farm combat at a
    // quarter of the fleet's gold rate for no return. Chase is now opt-in through /pin.
  ...
  releasePinIfDone() {
    ...
    if (Number.isFinite(tl) && tl >= REFERRAL.minLv) {
    ...
    if (mine < REFERRAL.minLv) return false;
