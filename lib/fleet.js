...
import { KINSHIP_RADIUS as KINSHIP_R, REFERRAL, BOSSES } from './rules.js';
...
export class Fleet {
  ...
  chooseRaid() {
    ...
    for (const [id, def] of golden) {
      ...
      const score = pay / Math.max(1, safe.secs);
      if (!best || score > best.score) {
        best = { bossId: id, x: def.x, z: def.z, members: safe.crew.map((b) => b.label), score,
                 pay, secs: safe.secs, go: false, at: Date.now() };
      }
    }
    return best;
  }
  ...
  syncRaid() {
    ...
    this.log(`raid called: ${next.members.length} on ${next.bossId} — ~${Math.round(next.pay)}g each, ~${Math.round(next.secs)}s of fighting`);
    return next;
  }
