...
export class Crafting {
  constructor({ net, state, move, log = console.log }) {
    ...
    net.on('monument', (m) => { this.monumentNeed = m?.need || null; });
  }
  ...
  async contributeToMonument({ want = 6 } = {}) {
    ...
    // The live monument frame names the tier's remaining requirement, so give it what it asks for.
    const wanted = Object.keys(this.monumentNeed || {}).length
      ? Object.keys(this.monumentNeed)
      : ['plank', 'ingot'];
    ...
    else this.log('[craft] the Monument refused what we offered');
    return given;
  }
