...
export class Net extends EventEmitter {
  ...
  // Drop the socket WITHOUT marking it deliberate, so the existing backoff + rejoin path runs and
  // `init` re-seeds the world. This is the only way back from a corrupted local position: the
  // server never echoes our own coordinates (snap deliberately skips us), so a rejoin is the
  // re-anchor.
  bounce() { if (this.ws) this.ws.close(); }
