...
import { QUEST_POOL, NOVICE_QUESTS, TUTORIAL, WHEEL, STAR, TREASURE, SLAYER, BUILDINGS, SEEDS } from './rules.js';
...
export class Quests {
  ...
  focusQuest() {
    ...
    for (const q of this.open) {
      const def = this.defOf(q.id) || q;
      if (!this.activityFor(q)) continue;              // nothing we know how to work on
      ...
      if (score > bestScore) { bestScore = score; best = { ...this.activityFor(q), quest: q, frac, remaining, score }; }
    }
    return best;
  }
