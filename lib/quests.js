...
export class Quests {
  ...
  focusQuest() {
    ...
    for (const q of this.open) {
      ...
      // A repeatable with a low bar is *always* nearest-to-done, so raw closeness let one quest
      // (Monument, 6 goods) take 144 of 194 focus picks in a 2 h run while 35 one-time quests sat
      // untouched. Decay a quest by how often we have already banked it, floored so a genuinely
      // good repeatable (62g + 210xp per ~2 min) still gets picked once the one-time board is dry.
      const claims = this._claims.get(q.id) || 0;
      const repeat = claims ? Math.max(0.15, 1 / (1 + claims)) : 1;
      const score = (frac * 1000 + value / remaining) * repeat;
