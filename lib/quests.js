...
export class Quests {
  constructor({ net, state, move, log = console.log }) {
    ...
    this._claims = new Map();   // quest id -> times claimed this session (repeatables recur)
    this.slayer = null;
    this.treasure = null;
    this._starActive = null;

    net.on('quest', (m) => { const q = m.list ?? m.quests; if (Array.isArray(q)) this.state.quests = q; });
  ...
  async claimReady() {
    ...
    for (const q of this.ready) {
      ...
      this._claims.set(q.id, (this._claims.get(q.id) || 0) + 1);
  ...
  focusQuest() {
    ...
    for (const q of this.open) {
      ...
      // Two rounds of field data on this. `frac * 1000` made closeness dominate absolutely, so a
      // repeatable with a low bar — always nearest-to-done — took 144 of 194 focus picks while 35
      // one-time quests sat at zero. Adding a repeat-decay spread the load across three repeatables
      // but still never started a fourth quest: at 0 progress the frac term is 0, so an unstarted
      // quest could never outscore a half-done one, whatever it paid. That is a cold start, not a
      // preference.
      //
      // So price the thing that actually matters — reward per unit of work still owed — and let
      // closeness bias it rather than decide it. A half-done quest gets up to 2x, and a repeatable
      // decays by how often we have already banked it (floored, because 62g + 210xp per ~2 min is
      // genuinely good once the one-time board is dry).
      const claims = this._claims.get(q.id) || 0;
      const repeat = claims ? Math.max(0.15, 1 / (1 + claims)) : 1;
      const score = (value / remaining) * (1 + frac) * repeat;
