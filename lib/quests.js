...
import { QUEST_POOL, NOVICE_QUESTS, TUTORIAL, WHEEL, STAR, TREASURE, SLAYER, BUILDINGS } from './rules.js';
...
export class Quests {
  ...
  activityFor(q) {
    ...
    switch (def.kind) {
      case 'gather':  return { activity: 'gather', skill: def.skill, item: def.item };
      ...
      case 'contribute': return { activity: 'contribute' };
      default:        return null;
    }
  }

  // Which activity buys us the most unclaimed quest value per unit of remaining work. The
  // orchestrator blends this with the daily-cap picture rather than obeying it blindly.
  preferredActivity() {
    let best = null, bestScore = 0;
    for (const q of this.open) {
      const def = this.defOf(q.id) || q;
      const remaining = Math.max(1, (q.need || def.need || 1) - (q.prog || 0));
      const value = (q.gold || def.gold || 0) + (q.xp || def.xp || 0) * 0.25;
      const score = value / remaining;
      const mapped = this.activityFor(q);
      if (mapped && score > bestScore) { bestScore = score; best = { ...mapped, quest: q, score }; }
    }
    return best;
  }
