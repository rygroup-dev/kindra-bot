...
import { walletOverview } from './chain.js';
...
// Always the bot's own client — see Bot.kgoldClient. A panel-local instance would have its own
// `mine` and would double-report every sale.
function kgoldOf(b) { return b.kgoldClient(); }
...
async function kgoldPanel(label) {
  ...
  return {
    text: [
      ...
      `auto cash-out: keeps *${CFG.goldReserve}g* in reserve, lists *${CFG.cashOutLot}g* lots at the going rate`,
      kg.sales.lots ? `sold so far this run: *${kg.sales.lots}* lot(s) · ${short(kg.sales.gold)}${E.gold} → *${kg.sales.kindra}* $KINDRA` : '',
      '',
    ...
    keyboard: KB([
      [[`${E.sell} List surplus (keep ${CFG.goldReserve}g)`, `cashout:${b.label}:${CFG.goldReserve}`]],
      [[`keep 5k`, `cashout:${b.label}:5000`], [`keep 20k`, `cashout:${b.label}:20000`]],
...
const ROUTES = {
  ...
  cashout: async (a) => {
    const b = bot(a[0]);
    const keep = parseInt(a[1] || String(CFG.goldReserve), 10);
    const r = await (await kgoldOf(b)).cashOutSurplus({ keepGold: keep });
