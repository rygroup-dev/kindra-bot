...
import { levelForXp, ITEMS } from './rules.js';
...
function homePanel() {
  ...
  const rows = [...fleet.bots.values()].map((b) => {
    const me = b.state.me;
  ...
  const queued = [...fleet.bots.values()].filter((b) => b.status === 'queued').length;
  const proxied = [...fleet.bots.values()].filter((b) => b.proxy).length;
  return {
    text: [
      `${E.brand} *Kindra Valley* — ${esc(world.name)}`,
      `${E.fleet} *${online}/${n}* character${n === 1 ? '' : 's'} in the valley${fleet.shiftsOn ? ` · ${E.clock} shifts on` : ''}`,
      '',
      box(rows),
    ...
    keyboard: KB([
      ...
      [[fleet.shiftsOn ? `${E.clock} Shifts: ON` : `${E.clock} Shifts: off`, 'shifts'], [`${E.acct} New account`, 'newmenu']],
...
function acctPanel(label) {
  ...
  return {
    text: [
      ...
      `🤝 account Lv ${me.tl ?? '?'}${(me.tl ?? 0) >= 10 ? ' — can refer (500g + 200 $KINDRA per head)' : ' — referring unlocks at Lv 10'}`,
    ...
    keyboard: KB([
      ...
      [[`🤝 Referrals`, `referral:${b.label}`], [`${E.cap} Caps`, `caps:${b.label}`]],
...
function questsPanel(label) {
  ...
  const rows = qs.map((q) => {
    const mark = q.claimed ? '✅' : (q.prog >= q.need ? '🎁' : '·');
    return `${mark} ${String(q.label).slice(0, 26).padEnd(26)} ${String(q.prog || 0).padStart(3)}/${q.need}`;
  });
...
function bossPanel(label) {
  ...
  return {
    text: [
      ...
      box(rows),
      target ? `${E.on} joining *${esc(target.name || target.id)}* — ${target.helpers} others already on it` : `${E.clock} nothing worth joining: the brain only engages a boss someone else is tanking`,
      `fights ${b.bosses.stats.fights} · damage ${short(b.bosses.stats.damage)} · spoils ${b.bosses.stats.spoils}`,
      '',
      `_Loot splits by damage contributed, so joining a fight in progress pays nearly as well as leading one at a fraction of the risk._`,
    ].join('\n'),
    keyboard: KB([navRow([[`${E.acct} ${esc(b.label)}`, `acct:${b.label}`]])]),
  };
}
...
const ROUTES = {
  ...
  newmenu: async () => ({
    ...
    keyboard: KB([[['+1', 'new:1'], ['+3', 'new:3'], ['+5', 'new:5'], ['+10', 'new:10']], [[`${E.back} Home`, 'home']]]),
  }),

  new: async (a) => {
    const n = Math.max(1, Math.min(20, parseInt(a[0] || '1', 10)));
    // Mint THROUGH a referrer when one qualifies: each conversion is worth 500 gold and 200 on-chain
    // $KINDRA to the referrer, and 250 gold to the newcomer. Minting without one throws that away.
    const ref = fleet.bestReferrer();
    const made = Fleet.createWallets(n, { world: world.id, referrer: ref?.name || null });
    ...
    return {
      text: [
        ...
        '',
        ref
          ? `🤝 referred by *${esc(ref.name)}* — pays ${esc(ref.label)} 500g + 200 $KINDRA each once they reach account Lv 5.`
          : `_No character has reached account Lv ${10} yet, so these were minted without a referrer. Mint more once one has — it is 500g + 200 $KINDRA per head._`,
        '',
        `Keys saved to \`${WALLET_FILE}\`.`,
      ].join('\n'),
      keyboard: KB([[[`▶️ Start all`, 'runall'], [`🤝 Referrals`, 'referral:']], [[`${E.back} Home`, 'home']]]),
      toast: `Minted ${n}${ref ? ' (referred)' : ''}`,
    };
  },

  referral: async (a) => {
    const b = bot(a[0]);
    await b.referral.refresh();
    const ref = fleet.bestReferrer();
    const referredBy = fleet.bots.get(b.label)?.referrer;
    return {
      text: [
        `🤝 *Referrals* — ${esc(b.state.me?.name || b.label)}`,
        box(b.referral.report().split('\n')),
        referredBy ? `joined through: *${esc(referredBy)}*` : '_joined without a referrer_',
        ref ? `fleet referrer: *${esc(ref.name)}* (${esc(ref.label)}, account Lv ${ref.tl})` : `_no character is account Lv 10 yet_`,
        '',
        `_The largest single source of on-chain token available to a fleet: every new character that reaches account Lv 5 pays its referrer 200 $KINDRA._`,
      ].join('\n'),
      keyboard: KB([
        [[`💰 Claim to wallet`, `claimref:${b.label}`], [`${E.refresh} Refresh`, `referral:${b.label}`]],
        navRow([[`${E.acct} ${esc(b.label)}`, `acct:${b.label}`]]),
      ]),
    };
  },
  claimref: async (a) => {
    const b = bot(a[0]);
    await b.referral.claim();
    return { ...(await ROUTES.referral([b.label])), toast: 'claim sent — pays to the linked wallet' };
  },
  ...
  shifts: async () => {
    ...
    fleet.startShifts();
    return { ...homePanel(), toast: `rotating every 45 min` };
  },
  ...
  where: async (a) => {
    ...
    return { text: `${E.pin} *${esc(me?.name || b.label)}* @ (${me?.x?.toFixed(1)}, ${me?.z?.toFixed(1)})\n${activityLabel(b.orch.current)} on ${esc(b.world.name)}`, keyboard: KB([navRow([[`${E.acct} ${esc(b.label)}`, `acct:${b.label}`]])]) };
  },
