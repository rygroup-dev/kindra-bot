...
function fleetPanel() {
  ...
  return {
    text: [
      ...
      banned.length ? `\n🚫 *${banned.length} banned* and out of the rotation: ${banned.map((b) => `\`${esc(b.label)}\``).join(' ')}` : '',
    ].filter(Boolean).join('\n'),
...
const ROUTES = {
  ...
  new: async (a) => {
    const n = Math.max(1, Math.min(20, parseInt(a[0] || '1', 10)));
    // No referrer unless one was configured by hand (KINDRA_REFERRER in .env). This used to credit
    // whichever of our own characters had cleared Lv 10, which is self-referral from a single IP —
    // it got eleven converted characters and the referrer banned for a year.
    ...
    return {
      text: [
        ...
        '',
        ref
          ? `🤝 referred by *${esc(ref.name)}* — set by hand in \`.env\`.`
          : `_Minted without a referrer._`,
        '',
        run.started.length
  ...
  referral: async (a) => {
    ...
    return {
      text: [
        ...
        ref ? `configured referrer: *${esc(ref.name)}* — from \`KINDRA_REFERRER\` in .env` : `_no referrer configured — new characters are minted without one_`,
        '',
        `⚠️ *Do not refer your own fleet.* A referrer is paid 500 gold + 200 $KINDRA once a character it referred reaches account Lv 5 — and the game bans for it. This fleet credited its own Lv-10 character; the eleven that converted were banned for a year, and so was the referrer, the same night. It is only safe with a genuinely separate account on a different connection.`,
      ].join('\n'),
      keyboard: KB([
