...
import {
  ...
  NODE_TYPES, COMBAT, JOBS, BOSSES, creatureStats, bossXp,
} from './rules.js';
...
export class Orchestrator {
  constructor({ net, state, move, gather, combat, economy, crafting, quests, upgrades, bosses, jobs, garden, realms, log = console.log }) {
    Object.assign(this, { net, state, move, gather, combat, economy, crafting, quests, upgrades, bosses, jobs, garden, realms, log });
    this.running = false;
    this.current = 'idle';
    this.cycle = 0;
    this.startedAt = 0;
  ...
  scoreActivities() {
    ...
    const out = [];
    const add = (activity, value, why, extra = {}) => out.push({ activity, score: Math.max(0, value), why, ...extra });
    ...
    const questBonus = (activity, skill = null) => {
      ...
      const gold = (q.quest.gold || 0) + (q.quest.xp || 0) * XP_GOLD;
    ...
    for (const skill of ['woodcutting', 'mining', 'foraging', 'fishing']) {
      ...
      // A node is worth its gold plus its XP, over the time to walk to it and work it.
      const def = NODE_TYPES[nodes[0].type] || {};
      const hits = def.hits || 1;
      const perNode = (def.gold || 0) + (def.baseXp || 0) * hits * XP_GOLD;
      ...
      if (this.chasing) v *= 0.35;   // gathering moves account level at a quarter weight
      add('gather', v + questBonus('gather', skill), `${nodes.length} node(s)`, { skill });
    }
    ...
    // --- combat ------------------------------------------------------------
    {
      ...
      if (!target) add('combat', 0, 'nothing safe to fight');
        ...
        let v = (gold + st.xp * XP_GOLD) * (60 / secs);
    ...
    for (const [activity, skill] of [['cook', 'cooking'], ['craft', 'crafting']]) {
      ...
      if (!recipes.length) { add(activity, 0, 'no materials'); continue; }
      const best = recipes.sort((a, b) => (b.xp || 0) - (a.xp || 0))[0];
      const n = this.crafting.canMake(best);
      const value = (best.xp || 0) * XP_GOLD * n;
    ...
    // --- garden ------------------------------------------------------------
    {
      ...
      if (!this.garden) add('garden', 0, 'unavailable');
        ...
        // Harvesting is immediate value; planting is value that lands later, so it is discounted.
        const harvestValue = ripe * (seed?.xp || 60) * XP_GOLD;
        const plantValue = free && seed ? free * seed.xp * XP_GOLD * 0.4 : 0;
        const v = (harvestValue + plantValue) * (60 / (AVG_WALK_SECS + 8));
        add('garden', v + questBonus('garden'), ripe ? `${ripe} ripe` : `${free} plot(s) free`);
      }
    }
    ...
    // --- bosses ------------------------------------------------------------
    {
      ...
      if (!boss) add('boss', 0, 'no boss with others on it');
        ...
        const gold = this.capLeft('boss') > 0 ? 400 : 0;
        add('boss', (gold + share * XP_GOLD) * (60 / 180), `${boss.id}, ${boss.helpers} others`);
      }
    }
    ...
    // --- trade roads -------------------------------------------------------
    {
      if (!this.jobs?.unlocked()) add('job', 0, `unlocks at Lv ${JOBS.unlockLv}`);
        ...
        else add('job', ((tier.sell - tier.cost) + tier.xp * XP_GOLD) * (60 / 180), tier.name);
      }
    }
