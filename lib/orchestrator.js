...
import {
  GATHER, levelForXp, KINSHIP_RADIUS, accountXpOf, xpForAccountLevel,
  NODE_TYPES, COMBAT, JOBS, BOSSES, creatureStats, bossXp, ACCOUNT_XP_OTHER, GOODS,
} from './rules.js';

// The account-level gate this project chases. Same number as fleet.js and bot.js; it is 10 because
// that is where the account-level content opens, not because of referrals — those are gone.
...
export class Orchestrator {
  constructor({ net, state, move, gather, combat, economy, crafting, quests, upgrades, bosses, jobs, garden, realms, shrine, kgoldFor = null, log = console.log }) {
    Object.assign(this, { net, state, move, gather, combat, economy, crafting, quests, upgrades, bosses, jobs, garden, realms, shrine, kgoldFor, log });
    this.running = false;
    this.current = 'idle';
    this.cycle = 0;
    this.startedAt = 0;
  ...
  // --- account-level chase -------------------------------------------------
  // Account Lv 10 is the gate this project still chases — not for referrals (removed; see the note
  // in config.js) but because it opens the trade roads at Lv 15 and the account-level content
  // behind it. ACCOUNT_XP_WEIGHT scores combat at 1.0 and every other skill at 0.25, so a character
  ...
  get chasing() {
    ...
    if (tl >= ACCOUNT_GATE_LV) {
      if (this._wasChasing) {
        this._wasChasing = false;
        this.chaseAccountLevel = false;
        this.log(`[orch] account Lv ${tl} reached — back to normal rotation`);
  ...
  chaseProgress() {
    ...
    const need = xpForAccountLevel(ACCOUNT_GATE_LV);
  ...
  scoreActivities() {
    ...
    // --- bosses ------------------------------------------------------------
    {
      ...
      if (raid) {
        ...
        const boss = this.bosses?.pick({ requireHelpers: true, capLeft: this.capLeft('boss') });
        if (!boss) add('boss', 0, 'no boss with others on it');
          ...
          // What THIS boss pays, plus what its xp OPENS. Four of the eleven carry no goldRoll at
          // all — the Grove Warden and Gloamroot, the two a lone character can safely join, pay a
          // gem and a mat and nothing else — so a flat number sent characters across the map for a
          // purse that was never there. But scoring them at zero was the opposite mistake: the
          // Warden is req combat 0 and 900 xp a kill, four kills from the Lv-12 gate that opens the
          // 5,000/day purse. bosses.worth() prices both halves; see the ladder note in bosses.js.
          const w = boss.worth;
          const share = bossXp(BOSSES[boss.id]?.hp || 0) * 0.25;   // we contribute a slice, not all of it
          const why = w.unlock > 0
            ? `${boss.id}, ${boss.helpers} others — ${Math.round(w.gold)}g + ${Math.round(w.unlock)}/min toward combat Lv ${this.bosses.ladder()?.next?.reqCombat}`
            : `${boss.id}, ${boss.helpers} others`;
          add('boss', (w.gold + share * xpGold) * (60 / 180) + w.unlock, why);
        }
      }
    }
  ...
  async runOnce() {
    ...
    // 2. Urgent business.
    // Also when nothing was announced but we know we died somewhere — see reclaimSacks.
    if (this.state.sacks.size || this.state.deathSpot) { this.current = 'reclaim'; await this.combat.reclaimSacks(); }
  ...
  async cashOutSurplus() {
    ...
    try {
      // The bot owns the client (bot.kgoldClient); the fallback is for an Orchestrator built
      // standalone, as the tests do.
      if (!this.kgold) {
        if (this.kgoldFor) this.kgold = await this.kgoldFor();
        else { const { KGold } = await import('./chain.js'); this.kgold = new KGold({ net: this.net, state: this.state, log: this.log }); }
      }
      const r = await this.kgold.cashOutSurplus();
      this._lastCashOut = Date.now();
      // listGold() already logs the listing itself; repeating it here put the same line in the log
      // twice and, once listings became a notification, the same message in the chat twice.
      ...
      if (!r.listed && r.reason) this.log(`[kgold] not listing yet — ${r.reason}`);
      return r;
  ...
  report() {
    ...
    return [
      ...
      this.chasing ? `🎯 chasing account Lv ${ACCOUNT_GATE_LV} — ${this.chaseProgress().pct.toFixed(1)}%` : '',
