...
export class Orchestrator {
  ...
  scoreActivities() {
    ...
    for (const skill of ['woodcutting', 'mining', 'foraging', 'fishing']) {
      ...
      // Committing to ONE quest: the bonus scales with how close it is, so the last few items of a
      // nearly-done objective outrank starting anything else.
      if (q?.activity === 'gather' && q.skill === skill) s += 60 + 140 * (q.frac || 0);
    ...
    // --- combat: hard-capped at 2000 gold/day and needs food to be safe.
    {
      ...
      if (q?.activity === 'combat') s += 60 + 140 * (q.frac || 0);
    ...
    // --- cooking: cheap XP and it manufactures the rations combat needs.
    {
      ...
      let s = can ? 40 : 0;
      if (foodLow && can) s += 70;
      if (q?.activity === 'cook') s += 60 + 140 * (q.frac || 0);
    ...
    // --- crafting: converts dead-weight raws into value and clears satchel space.
    {
      ...
      if (q?.activity === 'craft') s += 60 + 140 * (q.frac || 0);
    ...
    // is the one way to actually lose here.
    {
      ...
      if (this.garden) {
        ...
        // A garden quest is only ever satisfied here, so it has to pull hard enough to get planted.
        if (q?.activity === 'garden') s = Math.max(s, 120 + 140 * (q.frac || 0));
      }
      scores.push({ activity: 'garden', score: s });
    }

    // --- shopping: a dedicated trip, not something we wait for a full satchel to trigger. Going
    // from bare-handed to a 260g iron sword is 2.3x damage; waiting for the next market run to
    // stumble into it can cost an hour of fighting at a quarter rate.
    {
      let s = 0;
      if (this.upgrades) {
        const w = this.upgrades.bestWeapon();
        const held = this.state.me?.appearance?.weapon;
        if (w && w.cost + 250 <= (this.state.me?.gold || 0)) {
          s = held ? 50 : 150;          // unarmed is urgent; an upgrade can wait for the next trip
          if (this.chasing) s += 60;    // the chase is combat, and combat is the weapon
        }
      }
      scores.push({ activity: 'shop', score: s });
    }
  ...
  async runOnce() {
    ...
    const focusBefore = focusPick ? { id: focusPick.quest.id, prog: focusPick.quest.prog ?? 0 } : null;
    const focus = this.quests.focusLabel();
    this.log(`[orch] cycle ${this.cycle} -> ${this.current}${focus ? ` [finishing: ${focus}]` : ''} (score ${pick.score.toFixed(0)}; ${ranked.slice(1, 4).map((r) => `${r.skill || r.activity} ${r.score.toFixed(0)}`).join(', ')})`);
    ...
    // Did the objective we committed to actually move? If not, say so — three dead cycles and the
    // quest is set aside so the board can keep clearing.
    if (focusBefore) {
