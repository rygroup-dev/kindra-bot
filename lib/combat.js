...
import { COMBAT, DEATH, creatureStats, attackDamage, attackCooldown, levelForXp, weaponDamage } from './rules.js';
...
export class Combat {
  ...
  dpsAgainst(level) {
    const wd = weaponDamage(this.state.me?.appearance?.weapon) || 0;
    const { dmg } = attackDamage(this.myCombatLevel(), wd, 1, 1);   // critRoll 1 => never crit, so this is the floor
  ...
  expectedDamageTaken(level, packSize = 1) {
    ...
    return (secsToKill / (COMBAT.creatureAtkCd / 1000)) * dmg * packSize;
  }
  ...
  canSafelyFight(level, packSize = 1) {
    ...
    return this.expectedDamageTaken(level, packSize) * margin < COMBAT.playerHp * this.hpFrac;
  }

  // Best XP-per-second target we can actually survive.
  pickTarget({ maxLevel = 99 } = {}) {
    ...
    for (const c of this.state.liveCreatures({ maxLevel: ceiling })) {
  ...
  async run({ stopWhen = () => false, maxLevel = 99, maxMs = 15 * 60 * 1000 } = {}) {
    this._stop = false;
    ...
    while (!this._stop && Date.now() - started < maxMs) {
      ...
      // picking a fight rather than eating through the fight.
      if (this.hpFrac < 0.6) {
      ...
      const target = this.pickTarget({ maxLevel });
      if (!target) {
        ...
        if (Date.now() - (this._lastNoTarget || 0) > 20000) {
          ...
          this.log(`[combat] nothing safe to fight (lvl ${this.myCombatLevel()}, food ${this.hasFood() ? 'yes' : 'no'}, hp ${(this.hpFrac * 100).toFixed(0)}%)`);
        }
