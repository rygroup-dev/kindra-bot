...
export class Bosses {
  ...
  // --- the ladder ---------------------------------------------------------------------
  // `bossXp(hp) === hp` — verified against live spoils (The Drowned King, 2,800 hp, paid every
  // contributor exactly 2,800 xp). So a boss is worth its own health bar in experience, and the
  // gold-paying tiers sit behind combat gates that that experience opens:
  //
  //     Grove Warden    req  0     900 xp    no gold    respawn  2.5 min
  //     Drowned King    req 12   2,800 xp       60g     respawn  3 min
  //     Rimewyrm        req 20   4,200 xp      240g     respawn  5 min
  //
  // Four Wardens is combat Lv 12. Two Drowned Kings is Lv 20. Under half an hour of boss fighting
  // opens the 5,000/day boss purse — five times the vendor cap, and the vendor is the ONLY cap this
  // fleet has ever actually filled. A boss we cannot yet wound is therefore not "locked": it is the
  // reason to fight the one we can.
  //
  // This is the whole reason the ladder stayed unclimbed. `pick()` scored on raw xp and ignored
  // gold, `chooseRaid` filtered to bosses with a goldRoll — so the Warden, the only rung a Lv-1
  // character can stand on, was invisible to both, and twenty accounts sat at combat Lv 1-8 unable
  // to touch a single paying boss.

  // Combat xp needed for a level. levelForXp is a ladder, not a formula, so invert it by search.
  xpForCombat(lv) {
    let lo = 0, hi = 5_000_000;
    while (lo < hi) { const m = Math.floor((lo + hi) / 2); if (levelForXp(m) >= lv) hi = m; else lo = m + 1; }
    return lo;
  }

  // The next PAYING boss above our combat level, and the xp still owed to reach it.
  ladder() {
    const lvl = this.myCombatLevel();
    const xp = (this.state.me?.skills || {}).combat || 0;
    const next = Object.values(BOSSES)
      .filter((d) => d.reward?.goldRoll && (d.reqCombat ?? 0) > lvl)
      .sort((a, b) => (a.reqCombat ?? 0) - (b.reqCombat ?? 0))[0];
    if (!next) return null;
    return { next, need: Math.max(0, this.xpForCombat(next.reqCombat ?? 0) - xp) };
  }

  // The best gold-per-minute we can already earn from a boss at this combat level. The baseline the
  // ladder's raise is measured against.
  bestPayingPerMin(dps) {
    const lvl = this.myCombatLevel();
    let best = 0;
    for (const d of Object.values(BOSSES)) {
      const roll = d.reward?.goldRoll;
      if (!roll || (d.reqCombat ?? 0) > lvl || !d.hp) continue;
      const perMin = ((roll[0] + roll[1]) / 2) / Math.max(0.2, d.hp / dps / 60);
      if (perMin > best) best = perMin;
    }
    return best;
  }

  // What a boss is worth per minute of fighting, counting BOTH what it pays and what its xp opens.
  // `groupDps` lets the fleet price a raid with its real crew instead of one character's swing.
  worth(def, { capLeft = Infinity, groupDps = null } = {}) {
    const dps = groupDps ?? this.dps();
    if (!(dps > 0) || !def?.hp) return { gold: 0, unlock: 0, mins: Infinity, perMin: 0 };
    const mins = Math.max(0.2, def.hp / dps / 60);
    const roll = def.reward?.goldRoll;
    const gold = (roll && capLeft > 0) ? (roll[0] + roll[1]) / 2 : 0;

    // The compounding half. If this kill's xp is on the way to a gate, each kill buys 1/N of the
    // raise waiting behind it, where N is the kills still owed. The Warden pays nothing and scores
    // zero on its own arithmetic — but four of them turn a character that cannot scratch a paying
    // boss into one farming a 5,000/day purse, and pricing that at zero is how it never happened.
    let unlock = 0;
    const rung = this.ladder();
    const mine = bossXp(def.hp);
    // Only worth climbing toward a purse that still has room in it today.
    if (rung && mine > 0 && rung.need > 0 && capLeft > 0) {
      const kills = Math.max(1, Math.ceil(rung.need / mine));
      const nr = rung.next.reward.goldRoll;
      const nextMins = Math.max(0.2, rung.next.hp / dps / 60);
      // The raise is what the next gate pays over the best boss income we can ALREADY reach — not
      // over what this particular candidate pays. Measured the second way, a boss paying nothing
      // scored a bigger unlock than one paying 60g for the identical climb, which is backwards: it
      // made the fleet prefer the rung that fed it least.
      const raise = ((nr[0] + nr[1]) / 2) / nextMins - this.bestPayingPerMin(dps);
      if (raise > 0) unlock = raise / kills;
    }
    return { gold, unlock, mins, perMin: gold / mins + unlock };
  }

  // A boss worth joining: alive, we meet its combat requirement, and (by default) someone else is
  // already holding its attention. Ranked on what it is WORTH — gold plus the gate its xp opens —
  // discounted by the walk, rather than on whichever health bar is fattest.
  pick({ requireHelpers = true, maxTravel = 400, capLeft = Infinity } = {}) {
    const lvl = this.myCombatLevel();
    ...
    for (const b of this.state.bosses.values()) {
      ...
      const w = this.worth(def, { capLeft });
      // Spread the walk over the fight the way every other activity is priced.
      const score = (w.gold + w.unlock * w.mins) / (w.mins + dist / this.move.speed / 60 + 0.5);
      if (score > bestScore) { bestScore = score; best = { ...def, ...b, helpers, dist, worth: w }; }
    }
    return best;
  }
  ...
  async raidRun(raid) {
    ...
    try {
      if (this.realms && realm !== cameFrom && !(await this.realms.travelTo(realm))) {
        raid.done = true;   // whatever is wrong will still be wrong next cycle — let the fleet re-choose
        return { fought: false, reason: `couldn't reach the ${realm} realm` };
      }
      const late = await this.muster(raid, def);
      if (late) { raid.done = true; return { fought: false, reason: late }; }
      await this.crafting.buffFor('combat');
