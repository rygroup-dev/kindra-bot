...
export class Telegram {
  ...
  // Send a file. Used for the wallet export, which is why it returns the message_id — the caller
  // schedules its deletion.
  async sendDocument(filename, content, caption = '', chatId = this.chatId) {
    ...
    const j = await res.json().catch(() => ({ ok: false }));
    if (!j.ok) { this.log(`[tg] sendDocument failed: ${j.description}`); return null; }
    return j.result?.message_id ?? null;
  }

  async deleteMessage(messageId, chatId = this.chatId) {
  ...
  async dispatchCallback(q) {
    ...
    // Acknowledge IMMEDIATELY, always. A callback query expires in seconds, and any render slower
    // than that produced "query is too old" — the spinner hung and the tap appeared to do nothing.
    // The result is reported afterwards as a toast-style message where it matters.
    const slow = ['sell', 'run', 'runall', 'cashout', 'fundgas', 'sweep', 'tend', 'cook', 'food', 'spin', 'buyup', 'claimref', 'shifts'].includes(name);
    this.call('answerCallbackQuery', { callback_query_id: q.id, text: slow ? 'Working…' : undefined }).catch(() => {});
    let notice = '';
    try {
    ...
    if (notice) await this.send(notice);   // already acknowledged above; report the outcome as a message
  }

  async edit(text, chatId, messageId, keyboard) {
    // Telegram REFUSES an edit whose text and markup are byte-identical to what is already shown.
    // On a dashboard that repeats — tapping Quests twice, or Refresh on an unchanged panel — the
    // tap then does visibly nothing at all, which reads as a dead button. A freshness stamp makes
    // every render differ, so the panel always repaints and the tap always registers.
    const stamped = `${String(text).slice(0, 3950)}\n\n\`↻ ${new Date().toTimeString().slice(0, 8)}\``;
    const body = { chat_id: chatId, message_id: messageId, text: stamped, parse_mode: 'Markdown', disable_web_page_preview: true };
