...
export class GameState extends EventEmitter {
  ...
  attach(net) {
    ...
    net.on('hp',  (m) => { if (this.me && m.id === this.me.id) this.me.hp = m.hp; });
    ...
    net.on('quest', (m) => { if (Array.isArray(m.quests)) this.quests = m.quests; });
    net.on('sackSpawn', (m) => this.sacks.set(m.id, m));
    net.on('sackGone',  (m) => this.sacks.delete(m.id));
