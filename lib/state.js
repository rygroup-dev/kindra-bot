...
export class GameState extends EventEmitter {
  constructor() {
    ...
    this.deathSpot = null;     // { x, z, at } — where we last fell, for a corpse run with no frame
  ...
  attach(net) {
    net.on('init', (m) => {
      ...
      // `init` carries a sacks array and it was the one collection never read out of it. A sack is
      // only announced by `sackSpawn` while we are there to see it — respawn in town, forty units
      // from the corpse, and the frame never comes. Seeding from init is how a character that
      // rejoins inside the 30-minute window learns its loot is still lying there.
      this.sacks.clear();      for (const k of m.sacks || []) if (k?.id != null) this.sacks.set(k.id, k);
    ...
    net.on('reward', (m) => {
      // The frame carries the xp it just paid — the client renders it as "+94 combat" — and we
      // dropped it on the floor. Our copy of `skills` only moved when an `inv` frame happened to
      // arrive, and `inv` fires on a PICKUP, not on every kill. So a character killing steadily
      // and looting nothing showed almost no progress: 144 kills worth roughly 11,000 combat xp
      // read as ~350, and the account-level chase looked stalled when it was not. Apply it here;
      // the next `inv` overwrites the lot with the server's own totals, so any drift self-corrects.
      if (m?.xp && m.skill && this.me) {
        this.me.skills = { ...(this.me.skills || {}) };
        this.me.skills[m.skill] = (this.me.skills[m.skill] || 0) + m.xp;
      }
    ...
    // Where we fell. Measured over 45 post-fix deaths: 11 sacks recovered, 0 walks failed — and 34
    // deaths that never produced a sack line at all, so the corpse run was never even attempted.
    // The reclaim depends on `sackSpawn`, and that frame does not reach a character the server has
    // already moved to town. Our own last position does not depend on any frame, so record it.
    net.on('ko', () => {
      if (this.me && Number.isFinite(this.me.x) && Number.isFinite(this.me.z)) {
        this.deathSpot = { x: this.me.x, z: this.me.z, at: Date.now() };
      }
    });
