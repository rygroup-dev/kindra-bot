...
import { accountXpOf, xpForAccountLevel } from './rules.js';

// Account Lv 10 is the last gate the fleet still cares about: it opens two realm portals. It also
// used to open referrals, which this bot deliberately no longer does.
...
export class Bot extends EventEmitter {
  constructor({ label, privateKey, world = CFG.world, name = '', character = null, proxy = null, appearance = null, onLog = null }) {
    super();
    this.label = label;
    ...
    this.status = 'idle';
    this.lastError = null;
    this.banned = null;         // { at, until, hours, reason } once the server refuses it as banned
    this.logLines = [];
    ...
    this.state.on('ready', () => {
      const poll = async () => {
        ...
        try {
          const kg = await this.kgoldClient();
          await kg.refresh();
          const r = kg.marketRate();
          if (r) {
            const sample = kg.book.filter((l) => l.gold).slice(0, 3)
    ...
    this.net.on('reject', (m) => {
      ...
      // A ban is not a transient refusal, and it is not "max 3 online" either — the account is gone
      // for the better part of a year. Left undistinguished it kept its place in the rotation, so
      // every circuit spent one of the three per-IP slots opening a socket that could only ever be
      // refused. Record it, with when it lifts, so the fleet can route around it permanently.
      const hrs = /banned/i.test(this.lastError) ? Number(/(\d+)\s*h/i.exec(this.lastError)?.[1] || 0) : null;
      if (hrs !== null) {
        this.banned = { at: Date.now(), until: Date.now() + hrs * 3600_000, hours: hrs, reason: this.lastError };
        this.log(`BANNED for ~${Math.round(hrs / 24)} days — taken out of the rotation`);
        this.emit('banned', { label: this.label, ...this.banned });
      }
    ...
    // And on a COMBAT level-up. Gear gates on combat level, so a character can own a crystal_sword
    // it bought at the wrong moment, or inherit one, and go on swinging an iron one until its next
    // join simply because it was one level short when equipOwned last ran. Re-checking costs one
    // frame and only sends `equip` when something better is actually available.
    this.state.on('levelup', (m) => {
      if (m?.skill !== 'combat') return;
      this.upgrades.equipOwned().catch(() => {});
    });
  ...
  // ONE $KINDRA client per character, shared by everything that touches the book: the market
  // poller, the orchestrator's cash-out, and the Telegram panel. There used to be three separate
  // instances — the poller's built with a silent logger — each keeping its own copy of `mine`.
  // Since a sale is detected by a listing disappearing from `mine`, that meant the same sale was
  // noticed up to three times and reported by whichever instance happened to have a real log.
  async kgoldClient() {
    if (!this.kgold) {
      const { KGold } = await import('./chain.js');
      this.kgold = new KGold({ net: this.net, state: this.state, log: this.log });
    }
    return this.kgold;
  }
  ...
  _watchGold() {
    ...
    this._goldTimer = setInterval(() => {
      ...
      const chase = this.orch.chaseAccountLevel ? (() => {
        ...
        return ` · account Lv ${this.state.me?.tl ?? '?'} (${pr.pct.toFixed(1)}% to Lv ${ACCOUNT_GATE_LV})`;
      })() : '';
  ...
  async connect() {
    ...
    await this.net.connect({
      ...
      // No `ref`. This bot does not do referrals — see the note in README. A join carries nothing
      // that credits anybody, on a new character or an old one.
    });
    await ready;
    this.status = 'online';
    // Account level belongs on this line. It gates two realm portals, and it was the one number
    // nothing ever wrote down — answering "what level is this account?" meant reconstructing it
    // from level-up lines and hoping the log went back far enough.
    const tl = this.state.me.tl || 1;
    // One decimal, matching the half-hourly [gold] line. Printing 77 here and 76.7 there made two
    // readings of the SAME number look like the chase had gone backwards, and account xp cannot go
    // backwards — so the apparent fall sent me hunting a bug that was a rounding difference.
    const pct = (100 * accountXpOf(this.state.me.skills || {}) / xpForAccountLevel(ACCOUNT_GATE_LV)).toFixed(1);
    this.log(`joined as ${this.state.me.name} (id ${this.state.me.id}) on ${this.world.name} — gold ${this.state.me.gold} · account Lv ${tl}${tl < ACCOUNT_GATE_LV ? ` (${pct}% to Lv ${ACCOUNT_GATE_LV})` : ''}`);
    return this;
  }
