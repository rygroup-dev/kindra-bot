...
export class Bot extends EventEmitter {
  ...
  workLeft() {
    ...
    const capsLeft = ['combat', 'boss', 'vendor', 'trade']
    ...
    // Weight a nearly-finished quest far above a barely-started one; finishing is what pays.
    const questValue = inProgress.reduce((a, q) => {
      const frac = (q.prog ?? 0) / Math.max(1, q.need);
      return a + (q.gold || 20) * (0.25 + 0.75 * frac);
    }, 0) + claimable * 200;
