...
export class Bot extends EventEmitter {
  ...
  _watchGold() {
    ...
    // Referral standing is NOT polled on join any more. Nothing in the fleet refers anything now,
    // so this was one more frame per join asking a question with a permanently empty answer. The
    // Telegram panel still asks on demand for anyone who configured a real referrer by hand.
  ...
  async connect() {
    ...
    // from level-up lines and hoping the log went back far enough. It also gated referrals, which
    // this fleet no longer uses.
    ...
    const pct = (100 * accountXpOf(this.state.me.skills || {}) / xpForAccountLevel(REFERRAL.minLv)).toFixed(1);
    this.log(`joined as ${this.state.me.name} (id ${this.state.me.id}) on ${this.world.name} — gold ${this.state.me.gold} · account Lv ${tl}${tl < REFERRAL.minLv ? ` (${pct}% to Lv ${REFERRAL.minLv})` : ''}`);
    return this;
  }
