...
export class Bot extends EventEmitter {
  ...
  _watchGold() {
    ...
    // Ask the server where our referrals stand, on every join of the character that is chasing the
    // Lv 10 gate. Until now the only thing that ever asked was the Telegram panel, so the fleet ran
    // for a day with no idea whether its 23 minted characters were registered against the referrer
    // at all — the one fact the whole chase depends on.
    this.state.on('ready', () => {
      if (!this.orch.chaseAccountLevel && !this.isPrimary) return;
      setTimeout(() => this.referral.refresh().catch(() => {}), 4000);
    });
