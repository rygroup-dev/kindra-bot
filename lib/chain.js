...
export class KGold {
  ...
  // -> token loop existed only on paper. The reserve is now a flat CFG.goldReserve (300 by
  // default) for every character in the fleet, new or old.
  //
  // The trade-off is real and worth knowing: a reserve this small cannot save up. Rations,
  // repairs and upgrades are all bought earlier in the same sell trip, so they are not starved —
  // but anything costing more than roughly one lot will never be affordable, because the surplus
  // is listed before it can accumulate. Raise KINDRA_GOLD_RESERVE, or use /cashout keep=<n>.
  workingFloat() {
